  ...
  <a-card>
    ...
    <div>
      <div style="width: 100%; padding: 5px 0px">
        <a-space>
          <a-dropdown-button @click="onAdd()" v-auth="`contract.add`"
            >添加合同</a-dropdown-button
          >
        </a-space>
      </div>
      <a-table
      ...
      >
        <a @click="onDetail(record)" slot="label" slot-scope="record">{{
          record.title
        }}</a>
        <div slot="action" slot-scope="record">
          <a-dropdown>
            ...
            <a-menu slot="overlay">
              ...
              <a-menu-item>
                <a @click="onEdit(record)" v-auth="`contract.edit`">
                  <a-icon type="edit" />
                  编辑
                </a>
              </a-menu-item>
              <a-menu-item>
                <a @click="onDelete(record)" v-auth="`contract.delete`">
                  <a-icon type="delete" />
                  删除
                </a>
              </a-menu-item>
...
export default {
  ...
  computed: {
    ...
    crumbParams() {
      return Object.assign({}, this.searchform, this.params);
    },
    dataSource() {
      if (this.data.length) {
  ...
  authorize: {
    onAdd: "contract.add",
    onEdit: "contract.edit",
    onDelete: "contract.delete",
    onAccount:'contract.list',
  },
  filters: {
    moneySum(records) {
      var number = 0;
  ...
  methods: {
    onLoad() {
      Promise.all([
        ...
        .then((response) => {
          ...
          this.customers = response[2].data.data;
          this.columns = [
            {
              title: "合同名称",
              sorter: true,
    ...
    onDetail(record) {
      this.onLink({
        ...
        currRoute: "contract-list",
        targetRoute: "contract-detail",
      });
    },
    ...
    onBack() {
      ...
      console.log('last.params',last.params);
