<template>
  <div style="padding: 0 30px">
    ...
    <div style="width: 100%; padding: 5px 0px">
      <a-space>
        <a-button  @click="addsite" type="primary" v-if="$auth('organization.add')">添加组织</a-button>
        <a-divider type="vertical" v-if="$auth('organization.add')"/>
        <a-dropdown-button @click="onAdd()" v-auth="`user.add`" v-if="$auth('user.add')" :disabled="!id">
          新建用户
        </a-dropdown-button>
        ...
        <a-dropdown :disabled="selectedRows.length ? false : true">
          <a-menu slot="overlay">
            ...
            <a-menu-item key="3" @click="settOrganization" v-if="$auth('user.editprimary')">
              <a-icon type="user" />
              设置所在组织
            </a-menu-item>
          </a-menu>
    ...
    <a-modal
      title="添加组织"
      :visible="sitevisible"
      :confirm-loading="siteconfirmLoading"
      @ok="handleOk"
      :maskClosable="false"
      @cancel="
        () => {
          (sitevisible = false), this.$refs.formssiteadd.reset();
        }
      "
    >
      <k-form-build
...
export default {
  ...
  created() {
    ...
    // this.jsonDatasite = this.params.jsonData
  ...
  methods: {
    init() {
      Promise.all([
            ...
            form_get({name: "organization_add"}),
        ]).then(res=>{
            // console.log('res',res)
            ...
            this.jsonDatasite = res[3].data.data.form;
            // 所有站点id
            this.policylists = [];
    ...
    onPullOrganization() {
      ...
      //   const that = this;
      //   this.$confirm({
      //     title: "确定要从该组织移除选定的帐号吗?",
      //     content: "注：如选定的帐号仅剩当前组织，则无法将此帐号从该组织移除！",
      //     okText: "确定",
      //     okType: "danger",
      //     cancelText: "取消",
      //     onOk() {
    ...
    onok() {
      console.log('recordes', this.recordes)
      this.$refs.formsadd
        .getData()
        .then((res) => {
          var datas = {};
          if (this.edittitle == "编辑用户") {
            datas = {
              ...
              mobile: res.mobile,
              policy: this.editId ? this.recordes.policy ? this.recordes.policy.id : undefined : undefined,
              primary: res.primary
            };
          } else {
    ...
    handleOk(){
      this.$refs.formssiteadd
        .getData()
        .then((res) => {
          organization_add(res)
            .then(() => {
              this.sitevisible = false;
              this.$refs.formssiteadd.reset();
              this.$emit("event", {
                method: "inits",
                params: {},
              });
