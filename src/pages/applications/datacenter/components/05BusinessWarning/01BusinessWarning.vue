    ...
    <div class="task_list">
      ...
      <div style="width: 100%; padding: 5px 0px;margin-top: 10px">
        <a-space>
          ...
          -->
          <!-- <a-button @click="onStatistics()" style="margin-left:5px">统计</a-button> -->
      ...
      >
        ...
        <!-- 营业额（同比）字段的插槽(样式自定义) -->
        <span slot="yearLv" slot-scope="text,record">
          <div v-if="text>=0">
            <a-statistic
            :value="text*100"
            :precision="2"
        ...
        <!-- 营业额（环比）字段的插槽(样式自定义) -->
        <span slot="monthLv" slot-scope="text,record">
          <div v-if="text>=0">
            <a-statistic
            :value="text*100"
            :precision="2"
        ...
        <span slot="nowNashui" slot-scope="text,record">
          ...
          <span v-if="record.nowNashui>=record.lastNashui">
            ...
            <br />
            <span style="font-weight:700;color:#d81e06">{{text}}</span>
          </span>
          <span v-else>
            <img src="../../images/arrowdown.png" style="width:20px" />
            <br />
            <span style="color:#37e736;font-weight:700">{{text}}</span>
          </span> 
          -->
          <div v-if="record.nowNashui>record.lastNashui">
            <a-statistic
            :value="((record.nowNashui-record.lastNashui)/record.lastNashui)*100"
            :precision="2"
            suffix="%"
            :value-style="{ color: '#cf1322',fontSize:'18px' }"
            >
          ...
          <div v-else>
            <a-statistic
            :value="((record.lastNashui-record.nowNashui)/record.lastNashui)*100"
            :precision="2"
            suffix="%"
            :value-style="{ color:'#3f8600',fontSize:'18px' }"
            >
        ...
        <span slot="actions" slot-scope="text, record">
          <div v-if="record.yingLv>=0.5">
            <div v-if="record.naLv>=0.5">
              <span style="color:#d81e06;font-size:15px">
                营业额涨幅大
                <br />
                纳税额涨幅大
              </span>
            </div>
            <div v-else-if="record.naLv<=-0.5">
              <span style="color:#d81e06;font-size:15px">营业额涨幅大</span>
              <br />
              <span style="color:#3f8600;font-size:15px">纳税额跌幅大</span>
            </div>
            <div v-else>
              <span style="color:#d81e06;font-size:15px">营业额涨幅大</span>
            </div>
          </div>
          <div v-else-if="record.yingLv<=-0.5">
            <div v-if="record.naLv>=0.5">
              <span style="color:#3f8600;font-size:15px">
                营业额跌幅大
                <br />
              </span>
              <span style="color:#d81e06;font-size:15px">
            ...
            <div v-else-if="record.naLv<=-0.5">
              <span style="color:#3f8600;font-size:15px">营业额跌幅大</span>
              <br />
              <span style="color:#3f8600;font-size:15px">纳税额跌幅大</span>
            </div>
            <div v-else>
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          align: "center",
          width: "80px",
          scopedSlots: { customRender: "yearLv" },
        },
        {
          title: "营业额（环比）",
          dataIndex: "monthLv",
          align: "center",
          width: "80px",
          scopedSlots: { customRender: "monthLv" },
        },
        {
          title: "年营业收入",
          dataIndex: "scale",
      ...
      industryLists:[],  // 我们在 data 中定义的一个数组，用来承接接口返回来的数据
    };
  },
  computed: {
    pagination: {
  ...
  methods: {
    ...
    // init 初始化方法
    init() {
      this.loading = true;  // 遮罩层的值置为 true，即开启状态
      manage_admin_warning(this.searchform)
      .then(res => {
        this.dataSource = res.data.data;
        this.loading = false  // 访问成功，遮罩层置为 false，即关闭状态
      })
      dict_tree()
      .then(res => {
        // 公司状态列表
    ...
    // 预警详情
    warningType(record){
      if(record.nowYingyee/record.lastlastYingyee>1.5) {
        return "营业额涨幅大"
      }else if(record.nowYingyee/record.lastlastYingyee<0.5) {
        return "营业额跌幅大"
      } else {
        return "无"
      }
