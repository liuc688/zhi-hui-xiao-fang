    ...
    <div class="task_list">
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <a href="#" @click="onReset(record)" style="color:red"><a-icon type="sync" /> 重置密码 </a>
...
export default {
  ...
  computed: {
    ...
    dynamicDated(){
      let result = {
        policyList:this.policyarr.map(item => {
  ...
  created() {
    ...
    this.county = (this.params&&this.params.county)?this.params.county:this.county;
    this.init();
    this.list();
  },
  methods: {
    ...
    onReset(record){
      ...
      that.$confirm({
        ...
        cancelText: "取消",
        onOk() {
          reset_password({ id: record.user[0].id })
          .then((res) => {
            if (res.status) {
    ...
    init() {
      ...
      ruser_paginate(Object.assign(this.searchform,{page:this.page,per_page:this.per_page}))
      .then(res => {
    ...
    // 该接口唯一的作用就是获取嘉明开发区企业策略的 id
    list(){
      policy_list({organization:'62f9aa4ab66242ea3349c2ee'})
      .then(res => {
        this.policyList = res.data.data.map(item => {
          if(item.name == "zdxf_qy") {
            return item.id
          }else {
            return ''
          }
        })
        this.policyList = this.policyList.filter(i=>i && i.trim())
      })
    },
    // 公司名查询
    onCounty(){
      ruser_paginate({page:1,per_page:1000})
      .then(res => {
        // 访问成功，遮罩层置为 false，即关闭状态
        this.loading = false;
    ...
    onAdd() {
      ...
      this.method = user_add;
      this.$refs.channelForm.enable(["username"])
      this.$refs.channelForm.show(["password"])
    },
    // 基本信息确定按钮
    handleOk(){
      ...
      .then(data => {
        if(data.password) {
          this.method(Object.assign(data,{id:this.id},{password:md5(data.password)}))
          .then(res => {
            ruser_create(Object.assign({user:data.username},{company:data.company},{policy:this.policyList[0]}))  // 这里的 policy 是对应企业的政策
            .then(item => {
    ...
    onEdit(record){
      this.visible = true;
      this.id = record.user[0].id;
      this.$nextTick(() => {
        this.$refs.channelForm.disable(["username"])
        this.$refs.channelForm.hide(["password"])
        this.method = user_edit;  // bases_edit 是一个编辑接口
        this.$refs.channelForm.setData({  // 这里是一打开弹窗时，需要想当前的弹窗中渲染当前该条数据的信息
          username: record.user[0].username,
          company:record.company[0].id,
          policy:record.user[0].policy,
          nickname: record.user[0].nickname,
          realname: record.user[0].realname,
          mobile: record.user[0].mobile,
        });
      });
    },
    // 企业信息删除
