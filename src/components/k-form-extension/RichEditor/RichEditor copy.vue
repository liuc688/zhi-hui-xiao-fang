<template>
  <div class="ueditor-wraps">
    ...
    >
      ...
      <a-button v-if="record.correction"
      ...
      <div class="error_words">
        <div class="error_s">
          <a-spin :spinning="spinning">
            <div class="text_wrong" v-html="searchs_word.text"></div>
          </a-spin>
        </div>
        <div class="word_s">
...
import { text_parser } from "@/services/user";
    ...
    beforeInit(editorId) {
        ...
        const btn = new window.UE.ui.Button({
            ...
            onclick: function(){
              that.editor = editor;
                that.$confirm({
                  title: '添加锚点',
                  content: (<a-input placeholder="请输入锚点标题" ref="anchor_title"/>),
                  okText: '确认',
                  cancelText: '取消',
                  onOk(close){
                    editor.focus();
                    const title = that.$refs.anchor_title.stateValue;
                    editor.execCommand('inserthtml',`<a class="suncool-anchor" id="${title}"></a>`)
                    close()
                  }
                });
            }
      ...
      window.UE.registerUI('missensitive',(editor, uname)=>{
        const btn = new window.UE.ui.Button({
            name: uname,
            title: '错敏排查',
            cssRules: "background-image: url('/static/UEditor/themes/default/images/missensitive.png') !important;background-size: cover",
            // cssRules: "background-position: -200px 0;",
            onclick: function(){
              that.sensitives()
      ...
      if (this.transitionwords) {
        ...
        text_parser({
          ...
          .then((Response) => {
            ...
            // for (var n = 0; n < this.activeKeys.length; n++) {
            //   for (var m = n + 1; m < this.activeKeys.length; m++) {
            //     if (this.activeKeys[n][0] < this.activeKeys[m][0]) {
            //       var temp = this.activeKeys[n];
            //       this.activeKeys[n] = this.activeKeys[m];
            //       this.activeKeys[m] = temp;
            //     }
            //   }
            // }
            ...
            for (var s = 0; s < this.activeKeys.length; s++) {
              ...
              var fillText =
                "<span class='missensitive'>" + this.activeKeys[s][1] + "</span>";
              ...
              // this.contents[this.activeKey].substring(0, this.activeKeys[s][0]) +
              // "<span style='color:red;'>"+ this.activeKeys[s][1] +"</span>" +
              // this.contents[this.activeKey].substring(this.activeKeys[s][0] + this.activeKeys[s][1].length);
            }
