...
export default {
  ...
  methods: {
    ...
    async close(event) {
      ...
      if (this.options.removeApps) {
        this.options.removeApps(this.options);
      }
    ...
    mini() {
      ...
      let domMinIndex = parseInt(dom.getAttribute("minindex"));
      if (domMinIndex < 0) {
      ...
      if (this.options.removeApps && this.options.addApps) {
        this.addStyle.display = "none";
      }
      this.maxMiniState = 1;
      if (this.options.addApps) {
        this.options.addApps(this.options);
      }
    },
    max() {
      //最大化窗口
