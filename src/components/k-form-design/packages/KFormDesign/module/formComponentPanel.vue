...
<template>
  <div class="form-panel">
    ...
    <a-form class="a-form-box k-form-build" :form="form" :layout="data.config.layout"
      :hideRequiredMark="data.config.hideRequiredMark" :style="data.config.customStyle">
      <draggable tag="div" class="draggable-box" v-bind="{
        group: 'form-draggable',
        ghostClass: 'moving',
        animation: 180,
        handle: '.drag-move'
      }" v-model="data.list" @add="deepClone" @start="dragStart($event, data.list)">
        <transition-group tag="div" name="list" class="list-main">
          <layoutItem class="drag-move" v-for="record in data.list" :key="record.key" :record="record"
            :config="data.config" :selectItem.sync="selectItem" :startType="startType"
            :insertAllowedType="insertAllowedType" :hideModel="hideModel" @dragStart="dragStart"
            @handleSelectItem="handleSelectItem" @handleCopy="handleCopy" @handleDelete="handleDelete"
            @handleColAdd="handleColAdd" @handleShowRightMenu="handleShowRightMenu" />
        </transition-group>
      </draggable>
    </a-form>
    <!-- 右键菜单 start -->
    <div v-show="showRightMenu" :style="{ top: menuTop + 'px', left: menuLeft + 'px' }" class="right-menu">
      <ul>
        <li @click="handleDownMerge">
          <a-icon type="caret-down" />向下合并
        </li>
        <li @click="handleRightMerge">
          <a-icon type="caret-right" />向右合并
        </li>
        ...
        <li @click="handleAddRow">
          <a-icon type="border-verticle" />增加一行
        </li>
      </ul>
    </div>
  </div>
    ...
    handleCopy(isCopy = true, data) {
      const traverse = array => {
        array.forEach((element, index) => {
          ...
          } else if (element.type === "card" || element.type === "custom-tabs") {
            // 卡片布局
    ...
    handleDelete() {
      ...
      const traverse = array => {
        array = array.filter((element, index) => {
          ...
          if (element.type === "card" || element.type === "batch" || element.type === "custom-tabs") {
            // 卡片布局
